...
async function checkState(task_id, task_state) {
  ...
  try {
    const [existingTaskState] = await connection.query(queryExistingTaskState, [
      task_id,
    ]);
...
exports.loadTasks = async (req, res) => {
  ...
  const queryExistingPlans = "SELECT plan_mvp_name, plan_colour FROM plan WHERE plan_app_acronym = ?";

  try {
    const [existingTasks] = await connection.query(queryExistingTasks, [app_acronym,]);
    const [existingPlans] = await connection.query(queryExistingPlans, [app_acronym]);
    let planObj = {};
    existingPlans.forEach((plan) => {
      Object.assign(planObj,
      { [plan.plan_mvp_name] : plan.plan_colour }
    )})
    const tasks = existingTasks.map((task) => ({...task, plan_colour: planObj[task.task_plan]}));
    ...
    res.status(500).send(`An error occured while loading tasks: ${error.message}`);
  }
};
...
exports.loadTask = async (req, res) => {
  ...
  try {
    const [existingTask] = await connection.query(queryExistingTask, [task_id]);
    res.status(200).json(existingTask[0]);
  } catch (error) {
    console.error("Error while loading task:", error);
    res.status(500).send(`An error occured while loading task: ${error.message}`);
  }
};
...
exports.createTask = async (req, res) => {
  ...
  try {
    ...
    const [rnumber] = await transaction.query(queryRnumber, [task_app_acronym]);
    ...
    await transaction.query(queryUpdateApp, [app_rnumber, task_app_acronym]);
    ...
    res.status(500).send(`An error occured while creating task: ${error.message}`);
  } finally {
...
exports.saveChanges = async (req, res) => {
  ...
  const time = new Date().toLocaleTimeString();
  const newNote = `**********\n[USER: ${username}, STATE: ${task_state}, DATE: ${date}, TIME: ${time}]\n${task_notes}\n\n`;
  const queryExistingTaskPlan = "SELECT COUNT(*) AS count FROM task WHERE task_id = ? AND task_plan = ?";
  const queryUpdateTaskPlan = "UPDATE task SET task_plan = ? WHERE task_id = ?";
  ...
  try {
    const [existingTaskPlan] = await connection.query(queryExistingTaskPlan, [task_id, task_plan]);
    if (existingTaskPlan.count < 0 && task_state === "Open" || "Done") {
      await connection.query(queryUpdateTaskPlan, [task_plan, task_id]);
    }
    if(task_notes) {
      await connection.query(queryUpdateTaskNotes, [newNote, username, task_id]);
    }
    ...
    res.status(500).send(`Error while updating changes: ${error.message}`);
  }
};
...
exports.promoteTask = async (req, res) => {
  ...
  if (!stateConsistent) {
    ...
    try {
      const [[existingTaskPlan]] = await connection.query(queryExistingTaskPlan, [
        task_id,
        task_plan,
      ]);
      ...
      if (app_permit_done) {
        const emailSent = await nodemailer.sendMail(app_permit_done, task_id);
        if (!emailSent) {
          return res.status(400).json({
            success: false,
      ...
      res.status(500).send(`Error while updating task notes: ${error.message}`);
    }
  }
};
...
exports.demoteTask = async (req, res) => {
  ...
  if (!stateConsistent) {
    ...
    try {
      ...
      if (existingTaskPlan.count === 0 && (task_state === "Open" || task_state === "Done")) {
        await connection.query(queryUpdateTaskPlan, [task_plan, task_id]);
